...
def Delete_Imported_Create_New(modelName, cnts_new, cnt_struct, cnt_coords, cntMaterial, arr_acc_pts, fillerConstraint):
...
#If GSs are present, this is needed; otherwise, comment and set N_GSs to 0
...
#-----------------------------------START: MAP CONSTRAINTS-----------------------------------#
#When the input file was generated by parts, the constraint Embed-i does not correspont to 
#part CNT-i. Even more so if there are GSs
#So, to find the constraint Embed-j that corresponds to CNT-i a dictionary is constructed
fillerConstraint = dict()
for i in range(N_CNTs+N_GSs):
    
    #Name of EE constraint
    constraint_str = 'Embed-'+str(i+1) 
    
    #Get the name of the hidden set
    hidSet = mdb.models[modelName].constraints[constraint_str].embeddedRegion[0]
    #print('hidSet={}'.format(hidSet))
    
    #Get instance id
    instID = mdb.models[modelName].rootAssembly.sets[hidSet].instances[0]
    #print('instID={}'.format(instID))
    
    #Get name of set
    instName = mdb.models[modelName].rootAssembly.featuresById[instID].name
    #print('instName={}'.format(instName))
    
    #Create dictionary instance->constraint
    fillerConstraint[instName] = constraint_str
...
Delete_Imported_Create_New(modelName, cnts_new, cnt_struct, cnt_coords, cntMaterial, arr_acc_pts, fillerConstraint)
