...
def Remesh_partial_GS(modelName, gs_part_str, selectedElementCode, eeMeshSize):
    ...
    mdb.models[modelName].parts[gs_part_str].setMeshControls(
        elemShape=TET, regions=mdb.models[modelName].parts[gs_part_str].cells, technique=FREE)
    ...
    mdb.models[modelName].parts[gs_part_str].setElementType(
        ...
        regions=(mdb.models[modelName].parts[gs_part_str].cells, ))
...
#Name of the job that had errors due to zero volume elements in GSs
jobName = 'VOL0160_CNT02GNP02'

#New job name
#newJobName = jobName+'-1'
newJobName = jobName

#Filename for status messages
print_file = newJobName + '.txt'
...
#Open database (it is assumed a datacheck was performed or the job submission failed so 
#that there is an odb file)
odb = openOdb(path=jobName+'.odb')

#Get the instances with zero elements
#NOTE: THIS SCRIPT IS INTENDED FOR A JOB THAT HAD THIS ERROR. OTHERWISE THIS COMMEND WILL THROW AN ERROR.
instanceNames = odb.rootAssembly.elementSets['ErrElemVolSmallNegZero'].instanceNames
    
#Close odb file as it is not needed anymore
odb.close()
...
for inst in instanceNames:
    #plog(inst)
    #plog(inst[0:len(inst)-2])
    
    #Get part name
    gs_part_str = inst[0:len(inst)-2]
    
    #Get part number (as an integer)
    #plog(''+gs_part_str[3:len(inst)])
    gs_i = int(gs_part_str[3:len(inst)])
    print(gs_i)
    
    #Delete embedded element constrain
    #First all CNT constraints were created, then all GS constrains were added
    #Thus the constraint number for the GS needs the number of CNTs to be added
    del mdb.models[modelName].constraints['Embed-'+str(N_CNTs+gs_i+1)]
    
    #Delete instance
    del mdb.models[modelName].rootAssembly.instances[inst]
    
    #Delete part
    del mdb.models[modelName].parts[gs_part_str]
    
    #Get instance name as if new
    gs_inst_str = gs_part_str+'-1'
    
    #Create part again
    Create_GS(modelName, sheetSize, gs_i, gs_part_str)
    ...
    #Move every GS to its right place in the RVE and then cut it (if its partially outside)
    Translate_Rotate_and_Cut_GS(modelName, gs_i, P0, corner, gs_part_str, gs_inst_str)
        
    #Assign material to GS
    Assign_Section(modelName, gsMaterial, gs_part_str)
    
    #Update instance name after cut
    gs_inst_str = gs_part_str+'-2'
...
#This seems to be required by Abaqus after modifying the mesh
mdb.models[modelName].rootAssembly.regenerate()
...
mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
    ...
    multiprocessingMode=DEFAULT, name=newJobName, nodalOutputPrecision=SINGLE, 
...
#Save input file
start = time.time()
mdb.jobs[newJobName].writeInput(consistencyChecking=OFF)
plog("Input file written: {} secs.\n".format(time.time()-start))

# lckFile = newJobName+'.lck'
