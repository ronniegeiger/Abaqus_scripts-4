...
def Create_Set_For_CNT_Points(modelName, cnt_i, cnt_rad, cnt_start, cnt_end, cnt_coords, cnt_str):
    ...
    for i in range(cnt_start, cnt_end+1):
        ...
        if cnt_i == 617 and i-cnt_start == 58:
            print(newNodes)
            print(newNodes[0])
            print(newNodes[1])
        #print('n=',i-cnt_start,' len(newNodes)=', len(newNodes))
        #newNodesArr = list(newNodes[0])
        ...
        if len(newNodes) > 1:
            ...
            #Append the labels of the extra nodes
    ...
    #         if n == 58:
    #             print('n={}:'.format(n))
    #             print('{} {} {}'.format(cnt_coords[cnt_start+n][0], cnt_coords[cnt_start+n][1], cnt_coords[cnt_start+n][2]))
    #             for j in allNodes[n]:
    #                 print(j)
    #print(labelsToRemove)
    ...
    if len(labelsToRemove) >= 1:
        ...
        #Create a set with the node labels that need to be removed
...
#Name of the job to be used 
jobName = 'VOL0160_CNT02GNP01'
...
plog('Reading csv files time: {} secs.\n'.format(time.time()-start0))
...
for cnt_i in range(1, N_CNTs+1):
    ...
    if n_nodes != N_p:
        ...
        for i in range(cnt_start, cnt_end+1):
            ...
            #Check if more than one node was found
            if len(newNodes) > 1:
        ...
        mdb.models[modelName].rootAssembly.SetByBoolean(
            name=node_set_str, 
            sets=(mdb.models[modelName].rootAssembly.sets[node_set_str], setToRemove, ), 
            operation=DIFFERENCE)
...
if createJob == 1:
    ...
    if submitJob == 1:
        ...
        plog("Time for Job execution: {} secs.\n".format(end-start))
